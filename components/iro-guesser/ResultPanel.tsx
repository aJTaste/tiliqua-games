...
function ChannelBar({
  ...
  const diffColor =
    absDiff <= 15 ? "#22c55e" : absDiff <= 40 ? "#f97316" : "#ef4444";
  const diffStr = diff > 0 ? `+${diff}` : diff === 0 ? "±0" : `${diff}`;

  return (
    <div className="flex items-center gap-2 w-full">
      <span className="font-mono-game text-[9px] text-[#aaa] w-3 shrink-0">
        {label}
      </span>
...
export default function ResultPanel({ result, onNext }: Props) {
  ...
  const channels = [
    {
      label: "R",
      actual: actualColor.r,
      user: userColor.r,
      barColor: "#d95555",
    },
    {
      label: "G",
      actual: actualColor.g,
      user: userColor.g,
      barColor: "#4caf7a",
    },
    {
      label: "B",
      actual: actualColor.b,
      user: userColor.b,
      barColor: "#4a8fd9",
    },
  ];
